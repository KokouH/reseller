...
if __name__ == "__main__":
	...
	for acc in accs.get_accounts():
		...
		all_sellOrders += sum(float(order["you_receive"].replace('$', '').replace(' USD', '').replace(' each', '')) * int(order['description']['amount']) for order in sell_orders)
		sleep(random() * 2 + 2)
